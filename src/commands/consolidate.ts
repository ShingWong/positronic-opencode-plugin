...
function resolveEngramSrc(): string {
  const candidates = [
...
export async function run(opts: { dir?: string; brain?: string; text: string; arousal?: number; json?: boolean } = {} as any): Promise<{ json: any; human: string }> {
  const dir = opts.dir || process.cwd();
  const cfg: any = (() => { try { return loadConfig(dir); } catch { return { brains: {}, live: false }; } })();
  ...
  if (out.status === 0 && out.stdout) {
    try { rep = JSON.parse(out.stdout.trim()); } catch { rep = { _err: out.stdout.trim().slice(0, 200) }; }
  } else {

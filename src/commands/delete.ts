...
export async function run(opts:{dir?:string,brain?:string,force?:boolean,json?:boolean}={}): Promise<{json:any,human:string}> {
  ...
  // Force: delete
  const before:any = JSON.parse(JSON.stringify(cfg));
  try { fs.rmSync(path.join(dir, ".positronic", "brains", name), { recursive: true, force: true }); } catch {}
  if (cfg.brains && cfg.brains[name]) { delete cfg.brains[name]; saveConfig(dir, cfg); }
  const after:any = (()=>{ try{ return loadConfig(dir);} catch{ return {brains:{}};} })();

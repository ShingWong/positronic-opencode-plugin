...
function pyInitBrain(projectDir: string, name: string, profile: string, embed: string) {
  const r = spawnSync("python3", [PY_SHIM, projectDir, name, profile, embed], { encoding: "utf-8" });
  if (r.status !== 0) throw new Error(`brains.py failed: ${r.stderr || r.stdout}`);
}

export interface WizardResult {
  json: { ok: boolean; warning?: string; brains: Record<string, { profile: string; embed: string }>; created: string[]; existing: string[]; configPath: string; live?: boolean };
  human: string;
}

export async function runWizard(
  projectDir: string,
  opts: { answers?: Array<{ name: string; profile: string; embed: string }>; force?: boolean; live?: boolean } = {},
...
Next: run with a specific --brain/--profile/--embed to create. If a brain already exists you will be warned to add --force (data will be lost).`;
  ...
  if (existingBrains.length > 0 && !opts.force) {
    ...
    const brains: Record<string, { profile: string; embed: string }> = {};
    for (const a of answers) brains[a.name] = { profile: a.profile, embed: a.embed };
  ...
  let existing: any = {};
  try { existing = loadConfig(projectDir); } catch { /* ignore parse errors, will overwrite */ }
  // Handle --no-live / --live flag if present in next call via opts
  const liveFlag: boolean | undefined = (opts as any).live;
...
export async function initRun(opts: { dir?: string; force?: boolean; json?: boolean; live?: boolean; brains?: Array<{ name: string; profile: string; embed: string }> } = {}): Promise<{ json: any; human: string }> {
  const dir = opts.dir || process.cwd();
  if (!opts.brains) {
    // No options → human-friendly help (no side-effects)
    const res = await runWizard(dir, { answers: undefined as any, force: opts.force, live: opts.live });
    return res;
  }

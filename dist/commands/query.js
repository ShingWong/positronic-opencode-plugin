...
const ENG_SRC = (() => {
    ...
    for (const c of cands) {
        if (__import_path_exists(c))
            return c;
    }
    return "/usr/local/devel/positronic/positronic-engram/engine/src";
})();
function __import_path_exists(p) {
    try {
        spawnSync("test", ["-d", p], { timeout: 1000 });
        return true;
    }
    catch {
        return false;
...
export async function runQuery(opts = {}) {
    ...
    const human = Array.isArray(parsed)
        ? parsed.map((h, i) => `  ${i + 1}. τ=${round(h?.tau ?? h?.first_seen_tau ?? 0, 2)} ${(h?.subject_norm || h?.canonical_name || "").slice(0, 60)}`).join("\n") || "(no results)"

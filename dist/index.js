...
const __posSessionDirs = new Map();
let __posSessionDirNoClientLogged = false;
export function setPosClient(client) { __posClient = client; }
export function v2ResetSessionDirs() {
...
export function toolDir(args, ctx) {
...
export function composeMarker(dir, sessionID) {
    const anchors = pai(["query", "--sql", ANCHOR_SQL, "--json"], { cwd: dir });
    const frags = (anchors.ok && Array.isArray(anchors.json?.results))
        ? anchors.json.results.map((r) => r?.t).filter((t) => typeof t === "string" && t.length > 0)
        : [];
    if (frags.length === 0)
        return `session compacted ${sessionID}`.trim();
    const objs = pai(["query", "--sql", OBJECT_SQL, "--json"], { cwd: dir });
    const names = (objs.ok && Array.isArray(objs.json?.results))
        ? objs.json.results.map((r) => r?.canonical_name).filter((n) => typeof n === "string")
        : [];
    let text = frags.slice(0, 4).join(" | ");
    if (names.length)
        text += ` objects: ${names.slice(0, 8).join(", ")}`;
...
const __posIngestSeen = new Set();
export function v2ResetIngestState() { __posIngestSeen.clear(); }
export async function ingestAssistantOnce(text, dir, tag) {
...
export async function setupV2(ctx) {
    ...
    try {
        if (ctx.command && typeof ctx.command.transform === "function") {
            await ctx.command.transform((editor) => {
                if (!editor || typeof editor.add !== "function") {
                    logIngest("v2 command editor.add unavailable, skip");
                    return;
                }
                    ...
                    editor.add({
                        ...
                        execute: (function (cc) {
                            return async (inv) => {
                                ...
                                try {
                                    await ctx.session.prompt({ sessionID: inv && inv.sessionID, text, delivery: (inv && inv.delivery) || "steer" });
                                }
                                catch (e) {
                                    logIngest("v2 command prompt err " + (e && e.message));
                                }
                            };
                        })(c),
                    });
        ...
        logIngest("v2 command register err " + (e && e.message));
    }
    ...
    if (globalThis.__positronicV2Abort) {
...
const pluginModule = { id: "positronic-opencode-plugin", server: pluginFactory, setup: setupV2 };

...
- **Project root (Fix 10/11)**: `setup` captures `PluginInput` once (`ctxRoot` → `setProjectRoot`). Resolution: `directory` → `worktree` → `location` → `project` → `POSITRONIC_PROJECT_DIR` → `~/.config/positronic/project` → file-location `projectDir()` (only claims a dir containing `.positronic/`) → `cwd`; all dir fallbacks (including `toolDir`) prefer it. On beta-17823 the real dir is `ctx.location.directory` (`worktree` is an API object, `client`/`directory` undefined).

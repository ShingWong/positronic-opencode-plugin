...
- **Project root (Fix 10)**: `setup` captures `PluginInput.directory`/`.worktree` once (`setProjectRoot`); all ingest/compaction/handler dir fallbacks prefer it, then `POSITRONIC_PROJECT_DIR`, then file-location `projectDir()` (which only claims a dir containing `.positronic/`), then `cwd`. Required for global installs where the plugin file isn't inside the project.
